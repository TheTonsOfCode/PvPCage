...
public abstract class SubCommand extends BukkitCommand {
    ...
    public void setMessagePrefix(String messagePrefix) {
        ...
        this.messagePrefix = Utils.fixColor(messagePrefix);
    }
    ...
    public SubCommand getSubCommand(String name) {
        for (SubCommand liteSubCommand : subCommands) {
    ...
    public void runSubCommand(SubCommand ext, boolean noFindMsg, String nameCommand) {
        if (nameCommand == null) {
            return;
        }

        SubCommand sub = getSubCommand(nameCommand);
    ...
    public boolean checkHelp(SubCommand ex, String name) {
        if (name.equalsIgnoreCase("help") || name.equalsIgnoreCase("?")) {
            if (HELP == null) {
                ...
                if (ex instanceof Command) {
                    if (usageMessage != null) {
                        HELP += b + " &fUsage: " + usageMessage + n;
                    }
                    Command lc = (Command) ex;
                    if (lc.getMinA() != null) {
                ...
                if (!subCommands.isEmpty()) {
                    ...
                    HELP += c;
                    for (SubCommand lsc : subCommands) {
    ...
    protected void setValues(Player player, String[] args) {
        this.player = player;
        this.args = args;
        this.len = args == null ? 0 : args.length;
        this.mainArg = null;
        if (len != 0) {
            this.mainArg = args[0];
        }
    ...
    protected void send(CommandSender sender, boolean prefix, String message) {
        sender.sendMessage((prefix && messagePrefix != null ? messagePrefix + " " : "") + Utils.fixColor("&f" + message));
    }
    ...
    protected void setArg(String mainArg) {
